...
const AuthManager = {
  ...
  // Show sign in form
  showSignInForm() {
    if (this.elements.signInForm && this.elements.joinForm) {
      ...
      this.elements.signInTab.classList.remove('border-transparent', 'text-gray-600');
      
      this.elements.joinTab.classList.remove('border-blue-600', 'text-blue-600');
      this.elements.joinTab.classList.add('border-transparent', 'text-gray-600');
    }
  },
  ...
  showJoinForm() {
    if (this.elements.signInForm && this.elements.joinForm) {
      ...
      this.elements.joinTab.classList.remove('border-transparent', 'text-gray-600');
      
      this.elements.signInTab.classList.remove('border-blue-600', 'text-blue-600');
      this.elements.signInTab.classList.add('border-transparent', 'text-gray-600');
    }
  },
  ...
  handleSignIn() {
    // Get form values
    const email = this.elements.signInEmail ? this.elements.signInEmail.value : '';
    const password = this.elements.signInPassword ? this.elements.signInPassword.value : '';
    ...
    // Simulate authentication (replace with actual API calls in production)
    const mockUser = {
      name: email.split('@')[0],
      email: email,
    ...
    // Save user to local storage (in a real app, you'd use tokens)
  ...
  handleJoin() {
    // Get form values
    const firstName = this.elements.firstName ? this.elements.firstName.value : '';
    const lastName = this.elements.lastName ? this.elements.lastName.value : '';
    const email = this.elements.joinEmail ? this.elements.joinEmail.value : '';
    const password = this.elements.joinPassword ? this.elements.joinPassword.value : '';
    const confirmPassword = this.elements.confirmPassword ? this.elements.confirmPassword.value : '';
    const termsAccepted = this.elements.termsCheckbox ? this.elements.termsCheckbox.checked : false;
    ...
    console.log('Registration with:', { firstName, lastName, email });
    
    // Simulate account creation (replace with actual API calls in production)
    const mockUser = {
...
document.addEventListener('DOMContentLoaded', () => {
  console.log('Auth.js: Document ready, initializing authentication');
  AuthManager.init();
});
